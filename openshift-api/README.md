...
### Health Check
```bash
curl https://hackathon-api-finclude.apps.hackathon.francecentral.aroapp.io/
```

### Get a File
```bash
curl https://hackathon-api-finclude.apps.hackathon.francecentral.aroapp.io/file/TestFile.txt
```

### List Files
```bash
curl https://hackathon-api-finclude.apps.hackathon.francecentral.aroapp.io/files
```
...
1. **Check pod status**:
   ```bash
   oc get pods -n finclude-official
   ```

2. **View application logs**:
   ```bash
   oc logs -n finclude-official deployment/hackathon-api
   ```

3. **Access debug endpoint**:
   ```bash
   ROUTE_URL=$(oc get route hackathon-api -n finclude-official -o jsonpath='{.spec.host}')
   curl -k https://$ROUTE_URL/debug | jq .
   ```

4. **Run diagnostic test**:
   ```bash
   oc exec -n finclude-official deployment/hackathon-api -- python /app/test-wif.py
   ```