...
async def proxy_task(
    ...
    async for message in client_websocket:
        try:
            ...
            # If the message contains 'submit', call browser-use MCP and send result back
            if isinstance(data, dict):
                # Check for text fields in the message
                text = ""
                if "text" in data:
                    text = data["text"]
                elif "setup" not in data and "bearer_token" not in data:
                    # Try to find text in nested fields (for text message payloads)
                    ...
                    async with stdio_client(server_params) as (read, write):
                        async with ClientSession(read, write) as session:
                            ...
                            if isinstance(content, TextContent):
                                ...
                                reply = str(content)
                            await client_websocket.send(json.dumps({"result": reply}))
                    continue  # Don't forward this message to Gemini
            if DEBUG:
                print("proxying: ", data)
